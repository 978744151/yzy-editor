...
import { DataSchema, filterTree, findTree, normalizeApi } from 'amis-core'
...
export const getArgsWrapper = (items, multiple = false, patch = {}) => ({
  type: 'combo',
  name: 'args',
  // label: '动作参数',
  multiple,
...
// 是否数据容器
export const IS_DATA_CONTAINER = `${JSON.stringify(DATA_CONTAINER)}.includes(data.__rendererName)`
...
const getCustomNodeTreeSelectSchema = (opts) => ({
  type: 'tree-select',
  name: 'path',
  label: '内存变量',
  multiple: false,
...
export const ACTION_TYPE_TREE = (manager) => {
        ...
        },
        {
          ...
          descDetail: (info) => {
            return (
              <div>
                打开
                <span className="variable-left variable-right">{info?.args?.pageName || '-'}</span>
                页面
              </div>
            )
          },
      ...
      children: [
        {
          ...
          schema: [
            ...
            },
            {
              ...
              onChange: (valu, oldValue, model, form) => {
      ...
      children: [
        {
          ...
          descDetail: (info) => {
            ...
            return (
              <div>
                发送
                <span className="variable-right variable-left">{apiInfo?.method || '-'}</span>
                请求：
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          schema: [
            ...renderCmptSelect('目标组件', true, (value, oldVal, data, form) => {
        ...
        },
        {
          ...
          schema: [
            ...
            // 组件变量
            {
              ...
              body: [
                ...renderCmptActionSelect('目标组件', true, (value, oldVal, data, form) => {
...
`,
        ...
        {
          actionLabel: '邮件',
          actionType: 'e-action', //写email会触发email事件
          description: '发送邮件事件',
          schema: {
            type: 'wrapper',
            className: 'p-none',
...
// 渲染组件选择配置项
export const renderCmptSelect = (componentLabel, required, onChange) => {
  return [
    {
      type: 'tree-select',
      name: 'componentId',
      ...
      onChange: async (value, oldVal, data, form) => {
        onChange?.(value, oldVal, data, form)
      },
    },
  ]
...
// 渲染组件特性动作配置项
export const renderCmptActionSelect = (componentLabel, required, onChange) => {
  return [
    ...renderCmptSelect(componentLabel || '选择组件', true, async (value, oldVal, data, form) => {
      // 获取组件上下文
      if (form.data.__nodeId) {
        if (form.data.actionType === 'setValue') {
          ...
          if (SELECT_PROPS_CONTAINER.includes(form.data.__rendererName)) {
            const contextSchema = await form.data.getContextSchemas?.(form.data.__nodeId, true)
...
export const COMMON_ACTION_SCHEMA_MAP = {
  ...
  refresh: {
    descDetail: (info) => <div>刷新页面</div>,
  },
  alert: {
    descDetail: (info) => <div>打开提示对话框</div>,
  },
  confirm: {
    descDetail: (info) => <div>打开确认对话框</div>,
  },
}

// 获取动作树中指定的动作
export const findActionNode = (actions, actionType) =>
  findTree(actions, (node) => node.actionType === actionType)

// 获取包含指定子动作的动作
export const findSubActionNode = (actions, actionType) =>
  findTree(actions, (node) => node.actions?.find((item) => item.actionType === actionType))

// 获取真实的动作类型
export const getActionType = (action, hasSubActionNode) =>
...
// 获取事件Label文案
export const getEventLabel = (events, name) =>
  events.find((item) => item.eventName === name)?.eventLabel

// 获取事件描述文案
export const getEventDesc = (events, name) =>
  events.find((item) => item.eventName === name)?.description

export const getEventStrongDesc = (events, name) =>
  events.find((item) => item.eventName === name)?.strongDesc

// 判断插件动作中是否存在指定动作
export const hasActionType = (actionType, actions) => {
  if (!Array.isArray(actions)) {
    return false
  }
  return !!actions?.find((item) => [item.actionType, 'component'].includes(actionType))
}
...
export const getPropOfAcion = (
  ...
  if (action.componentId) {
    // 优先从组件特性动作中找
    const node = findTree(allComponents ?? [], (item) => item.value === action.componentId)
    prop =
      node &&
      pluginActions[node.type]?.find((item) => item.actionType === action.actionType)?.[propName]
  }

  if (!prop) {
    prop = findActionNode(actionTree, action.actionType)?.[propName]
  }
  ...
  if (!prop) {
    ...
    if (propName === 'actionLabel') {
      ...
      prop =
        hasSubActionNode?.actions?.find((item) => item.actionType === action.actionType)?.[
          propName
        ] ?? commonActionConfig[action.actionType]?.[propName]
    }
  }
...
export const getOldActionSchema = (manager, context) => {
                ...
                children: ({ onChange, value, data }) =>
                  data.actionType == 'ajax' ? (
                    <div className="m-b">
                      ...
                      {value ? (
                        <Button size="sm" level="link" className="m-l" onClick={() => onChange('')}>
                          清空设置
                        </Button>
                      ) : null}
                    </div>
 ...
 * 获取事件动作面板所需属性配置
 */
export const getEventControlConfig = (manager, context) => {
  const isSubEditor = manager.store.isSubEditor
  // 通用动作配置
  const commonActions = manager?.config.actionOptions?.customActionGetter?.(manager)
  // 动作树
  ...
  const checkComponent = (node, action) => {
    ...
    node.isScopeContainer = !!manager.dataSchema.getScope(`${node.id}-${node.type}`)
  ...
  return {
    ...
    getContextSchemas: async (id, withoutSuper) => {
    ...
    getComponents: (action) => {
      ...
      const result = filterTree(finalCmpts, (node) => checkComponent(node, action), 1, true)
      return result
    },
    actionConfigInitFormatter: async (action) => {
      let config = { ...action }
      ...
      if (['setValue'].includes(action.actionType) && action.args?.value) {
        ...
        if (Array.isArray(action.args?.value)) {
          config.args.value = action.args?.value.reduce((arr, valueItem, index) => {
            if (!arr[index]) {
              arr[index] = {}
            }
        ...
        } else if (action.actionType === 'setValue' && typeof action.args?.value === 'string') {
          config.args['__containerType'] = 'all'
      ...
      // 获取动作专有配置参数
      const innerArgs = getPropOfAcion(
        action,
        'innerArgs',
        actionTree,
      ...
      if (config.actionType === 'reload') {
        ...
        if (
          ...
          (config.args && !Object.keys(config.args).length && config.data === undefined)
        ) {
      ...
      if (action.actionType === 'addItem') {
        if (Array.isArray(action.args?.item)) {
          const comboArray = (action.args?.item || []).map((raw) => objectToComboArray(raw))
          config.args = {
            ...config.args,
    ...
    actionConfigSubmitFormatter: (config, type, actionData) => {
      let action = { ...config, groupType: undefined }
      ...
      if (config.actionType === 'openDialog') {
        ...
        const setInitSchema = (groupType, action) => {
          if (groupType === 'dialog') {
        ...
        const chooseCurrentDialog = (action) => {
        ...
        // 编辑
          ...
          if (config.__dialogSource === 'new') {
            ...
            if (
              ...
              (config.__dialogSource === 'new' && actionData?.__dialogSource === 'current')
            ) {
              setInitSchema(config.groupType, action)
            } else {
        ...
        if (config.args?.hasOwnProperty('path')) {
          ...
          if (config.args?.__valueInput !== undefined) {
            ...
            if (
              ...
              let tempArr = []
              config.args?.value.forEach((valueItem, index) => {
                valueItem.item.forEach((item) => {
                  if (!tempArr[index]) {
                    tempArr[index] = {}
                  }
      ...
      if (action.actionType === 'addItem' && action.__rendererName === 'input-table') {
        const comboArray = (config.args?.value || []).map((combo) => combo.item || {})
        action.args = {
          ...action.args,
          item: comboArray.map((raw) => comboArrayToObject(raw)),
        }
